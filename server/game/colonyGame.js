...
const SPAWN_POSITIONS = [
  ...
  {x:1000, y:200},  {x:1000, y:1800},
  {x:200,  y:1000}, {x:1800, y:1000}
]
...
export class ColonyGame {
  constructor(map) {
    ...
    this.dayPhase    = 'day'
    this._colorIdx   = 0
    this._maxPlayers = 0   // track peak player count
  }
  ...
  addColony(playerId) {
    ...
    // Track the highest number of players ever in this game
    if (this.colonies.size > this._maxPlayers) {
      this._maxPlayers = this.colonies.size
    }
  ...
  attack(playerId, antId, targetId) {
    ...
    for (const [defId, defColony] of this.colonies) {
      ...
      if (def.hp <= 0) {
        ...
        if (def.caste === 'queen') {
          defColony.morale = Math.max(0, defColony.morale - 40)
        }
    ...
    for (const [playerId, colony] of this.colonies) {
      for (const ant of Object.values(colony.ants)) {
        ...
        if (ant.target && ant.caste !== 'queen') {
          const speed = CASTE_STATS[ant.caste].speed
  ...
  checkWinner() {
    // ── CRITICAL FIX: never declare a winner in a solo session ───────────────
    // Only check once at least 2 players have been in the game
    if (this._maxPlayers < 2) return null

    // Also need at least 2 colonies currently to have a meaningful check
    if (this.colonies.size === 0) return null

    const alive = [...this.colonies.entries()].filter(([, c]) => {
      return Object.values(c.ants).some(a => a.caste === 'queen')
    })

    // Winner = last colony with a living queen
